...
def main():
    ...
    parser.add_argument("--ip", required=True, help="The simulated peer IP (e.g., 127.0.0.2)") # for testing on own terminals for now
    ...
    def user_discovery():
        ...
        while True:
            ...
            if now - last_profile_time >= BROADCAST_INTERVAL:
                msg = build_profile_message(user_id, user_profile["username"], user_profile["status"])
    ...
    def on_receive(msg, addr):
        if addr[0] == ip: # filter out own broadcast messages
            return
